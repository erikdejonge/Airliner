...
-(void) menuBuyGame: (id) sender
{
  ...
  [[UIApplication sharedApplication] openURL:[NSURL URLWithString:@"http://itunes.apple.com/nl/app/airliner-usa/id353126558?mt=8"]];
}
...
-(void) updateHud: (CCLayer *) world_layer dt: (ccTime) dt mx: (float) mx my: (float) my airplane_rotation: (float) arot
{
  ...
  if ([gs getPlayerState] == GAMEOVER) {
    if (NO == game_over_menu_drawn) {
      ...
      menu_game_over          = [CCSprite spriteWithFile:@"menu-game-over-lite-bg.png"];
      menu_game_over.position = ccp(SCREENWIDTH / 2, SCREENHEIGHT / 2);
      [world_layer addChild:menu_game_over z:10 tag:kGameMenuBG];
      m_world_layer = world_layer;
      CCMenuItemImage *buygame = [CCMenuItemImage itemFromNormalImage:@"menu-game-over-lite-btn-buy.png" selectedImage:@"menu-game-over-lite-btn-buy-click.png" target:self selector:@selector(menuBuyGame:)];
      ...
      gameovermenu = [CCMenu menuWithItems:buygame, menui, nil];
      [gameovermenu alignItemsVerticallyWithPadding:1];
      gameovermenu.position = ccp(SCREENWIDTH / 2, 106);
    ...
    if (AUTOFLYING ==[gs getPlayerState]) {
      displaystring = [NSString stringWithFormat:@"Autopilot destination: %@", [gs getdestinationString]];
    }
    else {
      displaystring = [NSString stringWithFormat:@"Your destination: %@", [gs getdestinationString]];
    }
    ...
    if (time_tracker_smallmap >= 0.5) {
      ...
      CGPoint smallplane_position = ccp((SCREENWIDTH - (smallmap.contentSize.width)) - HUDMARGIN, HUDMARGIN);
  ...
  turbotimetracker += dt;
  if (([gs getTurbo] == 1) && (turbotimetracker > 1.6) && (turbo_btn_down == NO)) {
    [gs setTurbo:0];
    turbo_on.visible = NO;
    turbo.visible    = YES;
    turbo_btn_down   = NO;
...
-(void) initHud: (CCLayer *) world_layer
{
  ...
  smallmap.position = ccp((SCREENWIDTH - (smallmap.contentSize.width / 2)) - HUDMARGIN, HUDMARGIN + (smallmap.contentSize.height / 2));
  ...
  turbo.position    = ccp(HUDMARGIN + (turbo.contentSize.width / 2), HUDMARGIN + (turbo.contentSize.height / 2));
  turbo_on.position = ccp(HUDMARGIN + (turbo.contentSize.width / 2), HUDMARGIN + (turbo.contentSize.height / 2));
