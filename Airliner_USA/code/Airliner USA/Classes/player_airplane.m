...
-(void) calculateMovementVectorAutoPilot
{
  ...
  // with a stacked CCRotateTo action, which the cocos2d action manager corrupts
  // on modern iOS (rotation blew up to garbage, wrecking steering and the smoke
  // trail).
  while (deg < 0)    deg += 360;
...
-(void) updatePlayer: (ccTime) dt
{
  ...
  if ([gs getCloseToTarget]) {
    if (autopilot_time_tracker2 > 0.5) {
      [self calculateMovementVectorAutoPilot];
      autopilot_time_tracker2 = 0;
    }
