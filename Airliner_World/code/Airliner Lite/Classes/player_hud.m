...
-(void) menuBuyGame: (id) sender
{
  ...
  [[UIApplication sharedApplication] openURL:[NSURL URLWithString:@"http://itunes.apple.com/nl/app/airliner/id338084998?mt=8"]];
}
...
-(void) updateHud: (CCLayer *) world_layer dt: (ccTime) dt mx: (float) mx my: (float) my airplane_rotation: (float) arot
{
  ...
  turbotimetracker += dt;
  if (([gs getTurbo] == 1) && (turbotimetracker > 1.5) && (turbo_btn_down == NO)) {
    [gs setTurbo:0];
    turbo_on.visible = NO;
    turbo.visible    = YES;
    turbo_btn_down   = NO;
